...
Before creating a session, you should call the static {{domxref("LanguageModel.availability_static", "LanguageModel.availability()")}} method to determine whether the language model supports a given configuration on the current device and whether the therefore model components are already downloaded or can be downloaded. While static feature detection works as a baseline check, it doesn't tell you whether a given model supports all modalities you may need. For example, a given model may only support text and image content, but not audio content. Dynamic availability checking allows pages to adapt gracefully if data for the desired configuration can't be provided or is not downloaded. For example, you could display a download prompt or fall back to a cloud-based AI service, rather than creating a session only to have it fail.
